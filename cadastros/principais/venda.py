...
#Combo Produtos
campoSelect = TK.StringVar()
comboProdutos = ttk.Combobox(produtos, width=62, textvariable=(campoSelect))
...
btnaddProdutoVenda.place(width=45, height=35, x=410, y=12)
...
btnrmProdutoVenda.place(width=45, height=35, x=410, y=52)

#TreeVew Itens da Lista
tv = ttk.Treeview(produtos, selectmode="browse",column=("Produto","Valor","Qtde.","Sub-Total"),
                  show='headings',height=11)
tv.column("Produto", width=50, minwidth=0, stretch=NO)
tv.heading("#1", text='Produto')
tv.column("Valor", minwidth=0,width=250,stretch=NO)
tv.heading("#2", text='Valor')
tv.column("Qtde.", minwidth=0,width=100,stretch=NO)
tv.heading("#3", text='Qtde.')
tv.column("Sub-Total", minwidth=0,width=100,stretch=NO)
tv.heading("#4", text='Sub-Total')
tv.place(x=460,y=22)

#Nome Valor do Produto
lblValorProduto = Label(produtos,text= "Valor do Produto: ",font = "Calibri, 11",).place(x=1, y=42)
lblPrecoProduto = Label(produtos,text= "R$: 10,00 ",font = "Calibri, 22",).place(x=1, y=60)

#Nome Valor do Produto
lblQtdeProduto = Label(produtos,text= "Quantidade: ",font = "Calibri, 11",).place(x=200, y=42)
#Spinbox Quantidade Produto
valorSelecionado = TK.StringVar(value=1)
spinBox = ttk.Spinbox(produtos, from_=1,to=20,width=5, textvariable= valorSelecionado, wrap= True).place(x=200, y=65)

#Nome Sub-total Item
lblsubTotalItem = Label(produtos,text= "Sub-total Item: ",font = "Calibri, 11",).place(x=290, y=42)
lblsubTotalItem = Label(produtos,text= "R$: 30,00 ",font = "Calibri, 22",).place(x=290, y=65)


#Valores
valores = LabelFrame(master, text="Valores da Venda", width= 500, height=120)
valores.pack(expand="no", anchor='w')
...
lblRecebido = Label(valores,text= "Valor Recebido: ",font = "Calibri, 11",).place(x=200, y=1)
#Valor Recebido
txtRecebido= Entry(valores, bd=2, font=("Calibri, 12"), justify=LEFT)
txtRecebido.place(width=120, height=25, x=200, y=20)
#lbl Troco
lblTroco = Label(valores,text= "Troco: ",font = "Calibri, 11",).place(x=400, y=1)
lblResulTroco= Label(valores,text= "R$ 100,00 ",font = "Calibri, 15",).place(x=400, y=20)
#Combo Forma de pagamento
campoSelect = TK.StringVar()
comboFormaPgto = ttk.Combobox(valores, width=62, textvariable=(campoSelect))
comboFormaPgto.place(x=1, y=40)
